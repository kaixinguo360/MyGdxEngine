...
public class VertexMixer {
    ...
    private Array<VertexAttribute> targetAttributes = new Array<>();
    private int targetAttrNum = 0;
    private int targetVertexSize = 0;
    private Array<VertexAttributes> inputAttributes = new Array<>();
    private Array<Integer> inputVertexSize = new Array<>();
    private Array<Integer> inputPositionOffset = new Array<>();
    private Array<Array<Integer>> inputMaps = new Array<>();
    private Array<Array<Integer>> inputOffsets = new Array<>();
    ...
    public void addVertex(Vector3 pos) {
        ...
        for(int i=0; i<this.targetAttributes.size; i++) {
            ...
            if(v.usage == VertexAttributes.Usage.Position) {
                newAttr[0] = (float) pos.x;
                newAttr[1] = (float) pos.y;
                newAttr[2] = (float) pos.z;
            } else {
                attrProvider.setAttr(targetAttributes.get(i), (float) pos.x, (float) pos.y, (float) pos.z, newAttr);
            }
    ...
    public void buildToArray(float[] vs, int vsOffset) {
        ...
        for(int i=0; i<vertices.length; i++) {
            vs[vsOffset*targetVertexSize + i] = vertices[i];
        }
